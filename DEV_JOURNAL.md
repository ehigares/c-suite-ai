...
| 2026-03-10 | 9.5 hotfix | API keys stored as plaintext on disk (Fernet? False) causing 401 on decrypt | `save_config()` had an `else` branch that silently stored plaintext when `_fernet_key` was None. Fix: removed the silent fallback — now raises RuntimeError if Fernet key unavailable during save, preventing plaintext storage. Endpoint returns 401 with actionable message. |
